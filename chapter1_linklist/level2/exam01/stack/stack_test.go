...
func TestPush(t *testing.T) {
	...
	fmt.Printf("======= %s =======\n", "测试栈的存取")
	val, _ := stack.Pop()
	equal(t, "pop出的数据", []int{val}, []int{9})
	val, _ = stack.Pop()
	equal(t, "pop出的数据", []int{val}, []int{8})
	val, _ = stack.Pop()
	equal(t, "pop出的数据", []int{val}, []int{7})
	val, _ = stack.Pop()
	equal(t, "pop出的数据", []int{val}, []int{6})
	val, _ = stack.Pop()
	equal(t, "pop出的数据", []int{val}, []int{5})
	val, _ = stack.Pop()
	equal(t, "pop出的数据", []int{val}, []int{4})
	val, _ = stack.Pop()
	equal(t, "pop出的数据", []int{val}, []int{3})
	val, _ = stack.Pop()
	equal(t, "pop出的数据", []int{val}, []int{2})
	compare_slice = []int{0,1}
	equal(t, "栈内数据", stack.GetStackSlice(), compare_slice)

	fmt.Printf("======= %s =======\n", "测试栈空，不能取出")
	val, _ = stack.Pop()
	equal(t, "pop出的数据", []int{val}, []int{1})
	val, _ = stack.Pop()
	equal(t, "pop出的数据", []int{val}, []int{0})
	val, err = stack.Pop()
	if err != nil {
		fmt.Println("err: ", err)
	}
