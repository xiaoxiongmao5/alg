...
// 头结点为空，head.Next 第一个有效节点
func BuildList[T any](values ...T) *LinkNode[T] {
	head := &LinkNode[T]{}
	tmp := head
	for _, v := range values {
		node := &LinkNode[T]{Val: v}
		tmp.Next = node
		tmp = node
	}
	return head
...
// 头结点存储值，它自己是第一个有效节点
func BuildListNoHead[T any](values ...T) *LinkNode[T] {
	var head *LinkNode[T]
	tmp := head
	for _, v := range values {
		node := &LinkNode[T]{Val: v}
...
func DumpList[T any](headNode *LinkNode[T]) string{
	...
	id := 1
	for {
		ret = append(ret, fmt.Sprintf("%v[%v]", id, tmp.Val))
		tmp = tmp.Next
		id++
		if tmp == nil {
			break
...
func DumpListNoHead[T any](headNode *LinkNode[T]) string{
	if headNode == nil {
		return "empty"
	}
	var ret = make([]string, 0)
